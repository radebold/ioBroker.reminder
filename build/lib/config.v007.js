...
function normalizeKeywords(value, fallback) {
  ...
  const list = (text ? text.split(",") : fallback).map(entry => entry.trim().toLowerCase()).filter(Boolean);
...
function normalizeAdapterConfig(raw) {
  return {
    ...
    replyLinkPhone: normalizePhone(raw.replyLinkPhone),
...
function normalizeTasks(rawTasks) {
  ...
  for (let index = 0; index < rawTasks.length; index++) {
    ...
    const childReplyId = toStringValue(raw.childReplyId || raw.childParticipantId || raw.childChatId);
    const parentReplyId = toStringValue(raw.parentReplyId || raw.parentParticipantId || raw.parentChatId);
    const childSendNumber = normalizePhone(raw.childSendNumber || raw.childPhoneNumber || raw.childPhone || raw.childNumber || raw.childChatId);
    const parentSendNumber = normalizePhone(raw.parentSendNumber || raw.parentPhoneNumber || raw.parentPhone || raw.parentNumber || raw.parentChatId);
    result.push({
...
//# sourceMappingURL=config.v007.js.map
